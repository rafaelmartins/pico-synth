...
getter(ps_tui_t *tui)
{
    ...
    return synth->settings.presets[synth->_current_preset].adsr.decay;
}
...
{
    synth_t *synth = tui->ctx_data;
    synth_preset_set_adsr_decay(synth, synth->_current_preset, b);
}


#define TITLE "ADSR Decay"
#define _MAX 0xff
...
const ps_tui_screen_t screen_settings_preset_adsr_decay = {
...
const ps_tui_screen_t screen_settings_preset_adsr_decay_notif = {
    .type = PS_TUI_SCREEN_SELECT_BYTE,
